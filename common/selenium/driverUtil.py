...
class DriverUtil:
    ...
    def get_driver(cls,selenium_hub,browser_type):
        ...
        if browser_type=='ie':
            ...
            firefox_profile.set_preference('browser.download.dir', ReadWebUIConfig().web_ui_config.download_dir)
            firefox_profile.set_preference('browser.helperApps.neverAsk.saveToDisk',download_file_content_types)
            firefox_options = Firefox_Options()
            if ReadWebUIConfig().web_ui_config.is_firefox_headless.lower()=='true':
            ...
            prefs={'download.default_directory': ReadWebUIConfig().web_ui_config.download_dir,'profile.default_content_settings.popups':0}
            chrome_options.add_experimental_option('prefs',prefs)
            if ReadWebUIConfig().web_ui_config.is_chrome_headless.lower()=='true':
